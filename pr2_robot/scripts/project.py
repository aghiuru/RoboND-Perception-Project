...
# Exercise-2 TODOs:
    ...
    pcl_cloud_filtered_pub.publish(ros_cloud_filtered)
...
def pr2_mover(object_list):
    ...
    dropbox_dict = {}
    for i, box in enumerate(dropbox):
        name = box['name']
        group = box['group']
        ...
        dropbox_dict[group] = {'name': name, 'position': position, 'pose': pose}

    # TODO: Parse parameters into individual variables

    # TODO: Rotate PR2 in place to capture side tables for the collision map

    rospy.loginfo(dropbox_dict)

    # TODO: Loop through the pick list
    for i, obj in enumerate(pick_list):

        object_name = String()
        object_name.data = obj['name']
        object_group = obj['group']

        rospy.loginfo('Looking for {} {}'.format(object_name, object_group))

        # TODO: Get the PointCloud for a given object and obtain it's centroid
        index = labels.index(object_name.data)
        centroid = centroids[index]

        rospy.loginfo('Found at {} {}'.format(index, centroid))
        ...
        place_pose = dropbox_dict[object_group]['pose']
        arm_name = String()
        arm_name.data = dropbox_dict[object_group]['name']
        ...
        #     rospy.loginfo("Response: ", resp.success)

        # except rospy.ServiceException, e:
        #     rospy.loginfo("Service call failed: %s")

    # TODO: Output your request parameters into output yaml file
    send_to_yaml('actions-2.yaml', dict_list)
...
if __name__ == '__main__':
    ...
    pcl_cloud_filtered_pub = rospy.Publisher("/pcl_filtered", PointCloud2, queue_size=1)
